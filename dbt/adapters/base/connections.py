...
class BaseConnectionManager(object):
    ...
    def assign(self, conn, name):
        ...
        The caller is responsible for putting the acquired connection into the
        in_use collection.

        This is not thread-safe! The caller should hold the lock and conn
        should not previously have been in use.

        :param Connection conn: A connection, in any state.
        :param str name: The name of the connection to acquire as.
        """
        if name is None:
            name = 'master'
